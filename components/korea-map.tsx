...
interface CityData {
    cityId: number;
    cityName: string;
    weatherStatus: string; // 백엔드 Enum 이름 ('CLEAR', 'SNOW' 등)
    ptyCode: string;
}

// 컴포넌트 Props 타입 정의
interface KoreaMapProps {
    cities: CityData[]; // /api/v1/weather/map-data 응답 배열
    onCityClick: (city: CityData) => void; // 지역 클릭 시 실행될 함수
}

// --- SVG ID 와 City ID 매핑 객체 (1~23번 도시와 매핑되는 SVG ID만 포함) ---
const svgIdToCityIdMap: { [key: string]: number } = {
    // 서울특별시 (1)
    "강동구": 1, "송파구": 1, "강남구": 1, "서초구": 1, "관악구": 1, "동작구": 1, "영등포구": 1, "금천구": 1, "구로구": 1, "강서구": 1, "양천구": 1, "마포구": 1, "서대문구": 1, "은평구": 1, "노원구": 1, "도봉구": 1, "강북구": 1, "성북구": 1, "중랑구": 1, "동대문구": 1, "광진구": 1, "용산구": 1, "중구": 1, "종로구": 1, "성동구": 1,
    // 수원시 (2)
    "수원시_영통구": 2, "수원시_팔달구": 2, "수원시_권선구": 2, "수원시_장안구": 2,
    // 인천광역시 (3) - ID 짧게 수정됨
    "강화군": 3, "서구": 3, "계양구": 3, "부평구": 3, "남동구": 3, "연수구": 3, "중구_인천": 3, "옹진군": 3, "동구": 3, "미추홀구": 3,
    // 부산광역시 (4) - ID 짧게 수정됨
    "기장군": 4, "사상구": 4, "수영구": 4, "연제구": 4, "강서구_부산": 4, "금정구": 4, "사하구": 4, "해운대구": 4, "북구_부산": 4, "남구_부산": 4, "동래구": 4, "부산진구": 4, "영도구": 4, "동구_부산": 4, "서구_부산": 4, "중구_부산": 4,
    // 대구광역시 (5) - ID 짧게 수정됨
    "달성군": 5, "달서구": 5, "수성구": 5, "북구": 5, "남구": 5, "서구_대구": 5, "동구_대구": 5, "중구_대구": 5,
    // 광주광역시 (6) - ID 짧게 수정됨
    "광산구": 6, "북구_광주": 6, "남구_광주": 6, "서구_광주": 6, "동구_광주": 6,
    // 대전광역시 (7) - ID 짧게 수정됨
    "대덕구": 7, "유성구": 7, "서구_대전": 7, "중구_대전": 7, "동구_대전": 7,
    // 울산광역시 (8) - ID 짧게 수정됨
    "울주군": 8, "북구_울산": 8, "동구_울산": 8, "남구_울산": 8, "중구_울산": 8,
    // 세종특별자치시 (9) - ID 짧게 수정됨
    "세종특별자치시": 9,
    // 춘천시 (10)
    "춘천시": 10,
    // 청주시 (11)
    "청주시_청원구": 11, "청주시_흥덕구": 11, "청주시_서원구": 11, "청주시_상당구": 11,
    // 홍성군 (12)
    "홍성군": 12,
    // 전주시 (13)
    "전주시_덕진구": 13, "전주시_완산구": 13,
    // 무안군 (14)
    "무안군": 14,
    // 안동시 (15)
    "안동시": 15,
    // 창원시 (16)
    "창원시_진해구": 16, "창원시_마산회원구": 16, "창원시_마산합포구": 16, "창원시_성산구": 16, "창원시_의창구": 16,
    // 제주시 (17)
    "서귀포시": 17, "제주시": 17,
    // 성남시 (18)
    "성남시_분당구": 18, "성남시_중원구": 18, "성남시_수정구": 18,
    // 용인시 (19)
    "용인시_처인구": 19, "용인시_수지구": 19, "용인시_기흥구": 19,
    // 강릉시 (20)
    "강릉시": 20,
    // 고양시 (21)
    "고양시_일산서구": 21, "고양시_일산동구": 21, "고양시_덕양구": 21,
    // 천안시 (22)
    "천안시_서북구": 22, "천안시_동남구": 22,
    // 포항시 (23)
    "포항시_북구": 23, "포항시_남구": 23,
};

// SVG ID로 City ID를 찾는 헬퍼 함수
const getCityIdFromSvgId = (svgId: string): number | null => {
    // 긴 ID 처리 (필요한 것만 남김)
    if (svgId.startsWith("중구_000000902839")) return svgIdToCityIdMap["중구_인천"];
    if (svgId.startsWith("강서구_0000000596")) return svgIdToCityIdMap["강서구_부산"];
    if (svgId.startsWith("북구_00000147926")) return svgIdToCityIdMap["북구_부산"];
    if (svgId.startsWith("남구_00000052072")) return svgIdToCityIdMap["남구_부산"];
    if (svgId.startsWith("동구_00000168118")) return svgIdToCityIdMap["동구_부산"];
    if (svgId.startsWith("서구_00000120547")) return svgIdToCityIdMap["서구_부산"];
    if (svgId.startsWith("중구_00000099627")) return svgIdToCityIdMap["중구_부산"];
    if (svgId.startsWith("서구_00000095322")) return svgIdToCityIdMap["서구_대구"];
    if (svgId.startsWith("동구_00000147187")) return svgIdToCityIdMap["동구_대구"];
    if (svgId.startsWith("중구_00000163049")) return svgIdToCityIdMap["중구_대구"];
    if (svgId.startsWith("북구_00000166644")) return svgIdToCityIdMap["북구_광주"];
    if (svgId.startsWith("남구_00000057829")) return svgIdToCityIdMap["남구_광주"];
    if (svgId.startsWith("서구_00000160873")) return svgIdToCityIdMap["서구_광주"];
    if (svgId.startsWith("동구_00000183228")) return svgIdToCityIdMap["동구_광주"];
    if (svgId.startsWith("서구_00000070079")) return svgIdToCityIdMap["서구_대전"];
    if (svgId.startsWith("중구_00000094613")) return svgIdToCityIdMap["중구_대전"];
    if (svgId.startsWith("동구_00000077295")) return svgIdToCityIdMap["동구_대전"];
    if (svgId.startsWith("북구_00000009551")) return svgIdToCityIdMap["북구_울산"];
    if (svgId.startsWith("동구_00000153670")) return svgIdToCityIdMap["동구_울산"];
    if (svgId.startsWith("남구_00000116235")) return svgIdToCityIdMap["남구_울산"];
    if (svgId.startsWith("중구_00000032630")) return svgIdToCityIdMap["중구_울산"];
    if (svgId.startsWith("세종특별자치시_00000020366")) return svgIdToCityIdMap["세종특별자치시"];

    // 매핑 테이블에 직접 조회
    const cityId = svgIdToCityIdMap[svgId];
    return cityId === undefined ? null : cityId; // undefined (매핑에 없는 ID)면 null 반환
}

export function KoreaMap({ cities, onCityClick }: KoreaMapProps) {
    const [hoveredSvgId, setHoveredSvgId] = useState<string | null>(null);

    const getCityData = (cityId: number): CityData | undefined => {
        return cities.find((c) => c.cityId === cityId);
    }

    const isSnowing = (ptyCode: string): boolean => {
        return ptyCode === "3" || ptyCode === "7";
    }

    const getRegionStyle = (svgId: string): string => {
        const cityId = getCityIdFromSvgId(svgId);
        if (cityId === null || !cities || cities.length === 0) {
            return "fill-gray-700 stroke-gray-500 opacity-60 pointer-events-none";
        }

        const city = getCityData(cityId);
        if (!city) {
            return "fill-gray-600 stroke-gray-500 pointer-events-none";
        }

        const hovered = hoveredSvgId === svgId;
        const ptyCode = city.ptyCode;

        const isCurrentlySnowing = isSnowing(ptyCode);

        let baseStyle = "stroke-white stroke-[0.5] cursor-pointer transition-all duration-150";
        let weatherStyle = "";
        let hoverStyle = "";

        if (isCurrentlySnowing && ptyCode !== "-999") { // 눈 또는 눈날림 (정상 관측치)
            // 눈 올 때: 흰색 그라데이션 대신 하늘색 사용
            weatherStyle = "fill-sky-300";
            hoverStyle = hovered ? "brightness-125 opacity-100" : "opacity-85";
        } else if (ptyCode === "0") { // 맑음
            // 맑음일 때는 반짝일 필요 없으므로, 기본 회색보다 밝은 색 사용
            weatherStyle = "fill-gray-100";
            hoverStyle = hovered ? "brightness-110 opacity-100" : "opacity-90";
        } else { // 그 외 (비, 흐림, 결측치 등)
            // 기본 배경색: 어두운 회색
            weatherStyle = "fill-gray-800";
            hoverStyle = hovered ? "brightness-125 opacity-90" : "opacity-75";
        }

        return `${baseStyle} ${weatherStyle} ${hoverStyle}`;
    };

    // 그룹 애니메이션 함수: 눈이 올 때만 animate-pulse 적용 (수정!)
    const getGroupAnimation = (cityId: number | null): string => {
        if (cityId === null || !cities || cities.length === 0) return "";
        const city = getCityData(cityId);
        if (!city) return "";

        // [수정!] isSnowing 조건으로 변경
        const isCurrentlySnowing = isSnowing(city.ptyCode);

        // 눈이 오고 (3, 7) 결측치가 아닐 때만 애니메이션 적용
        if (isCurrentlySnowing && city.ptyCode !== "-999") {
    ...
    const handleRegionClick = (svgId: string) => {
        ...
        if (cityId) {
            ...
            if (city) {
                ...
                console.warn(`City data not found for cityId: ${cityId} (svgId: ${svgId})`);
            }
        } else {
            console.warn(`No cityId mapping found for svgId: ${svgId}`);
        }
    };
    ...
    return (
        <div className="relative w-full max-w-2xl mx-auto">
            <svg
                viewBox="0 0 509 716.1" // SVG 파일 viewBox 값 적용
        ...
        `}</style>
                {/* SVG Gradient Definition (Optional) */}
                <defs>
                ...
                <g
                    id="제주특별자치도" // 그룹 ID
                    className={`${getGroupAnimation(17)} cursor-pointer`} // 제주 cityId는 17
                    onMouseEnter={() => setHoveredSvgId("제주특별자치도_그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(17); if (city) onCityClick(city); }}
                >
                    <path
                        id="서귀포시"
                        className={`st0 ${getRegionStyle("서귀포시")}`}
                        d="M454.8,669.3l21.4-9.5l0.4,0.6l0.2,0l2.8,0.5l-0.6,3.2l1.7-3.6l1.2,3.3l-1.3,22.6l-8.1,0.8 l-4.3,4.4l-11.2,0.7l-7.8,5.9l-10.3-0.9l-6.5,2.3l-1.8-2.5l-11.5,1.5l-5.7-1.6l-7.5,7l-13.2-13.7l3.1-2l6.9,3.7l0-2.4l6.6-4.4 l6.1-4.1l8.4,0.1l4.3-0.6l0.4-1.4l5.8-0.3l5.2-0.4l7.4-1.8l0.1,0l1.6-0.4l12-5.3l3.6-1.8l0,0l3.4,0L454.8,669.3z"
                        onMouseEnter={() => setHoveredSvgId("서귀포시")} // 개별 호버 유지
                        onMouseLeave={() => setHoveredSvgId(null)}   // 개별 호버 유지
                    />
                    <path
                        id="제주시"
                        className={`st0 ${getRegionStyle("제주시")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("제주시")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                <g id="경상남도">
		...
		312.6,557 309.2,560.5 309.2,560.5 304.5,556.4 307.6,553.6 303.1,550.5 297.2,552.8 291.9,548.1 285.8,550.8 285.2,544.4 	"/>
                    <g
                        id="창원시그룹" // 그룹 ID
                        className={`${getGroupAnimation(16)} cursor-pointer`} // 창원 cityId는 16
                        onMouseEnter={() => setHoveredSvgId("창원시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(16); if (city) onCityClick(city); }}
                    >
                        {/* ID 짧게 수정됨 */}
                        <path
                            id="창원시_진해구"
                            className={`st0 ${getRegionStyle("창원시_진해구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("창원시_진해구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="창원시_마산회원구"
                            className={`st0 ${getRegionStyle("창원시_마산회원구")}`}
                            points="346.9,538.8 348.3,525.4 360.5,525.5 365.8,534 364.6,534.8 361.8,535.1 355,534.7 351.9,540.5"
                            onMouseEnter={() => setHoveredSvgId("창원시_마산회원구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <path
                            id="창원시_마산합포구"
                            className={`st0 ${getRegionStyle("창원시_마산합포구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("창원시_마산합포구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="창원시_성산구"
                            className={`st0 ${getRegionStyle("창원시_성산구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("창원시_성산구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="창원시_의창구"
                            className={`st0 ${getRegionStyle("창원시_의창구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("창원시_의창구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
                </g>
		...
		344.3,400.5 336,397.5 330.5,402.5 326.6,397.7 321.7,398.3 319.5,395.7 322.1,386 319,381.5 	"/>
                    <g
                        id="안동시그룹" // 그룹 ID
                        className={`${getGroupAnimation(15)} cursor-pointer`} // 안동 cityId는 15
                        onMouseEnter={() => setHoveredSvgId("안동시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(15); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="안동시"
                            className={`st0 ${getRegionStyle("안동시")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("안동시")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
		...
		l6.5,3.7l2.3-1.7l-3.1-12.2l4.2-0.8l-1.3-3.8L430.9,402.1z"/>
                    <g
                        id="포항시그룹" // 그룹 ID
                        className={`${getGroupAnimation(23)} cursor-pointer`} // 포항 cityId는 23
                        onMouseEnter={() => setHoveredSvgId("포항시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(23); if (city) onCityClick(city); }}
                    >
                        <path
                            id="포항시_북구"
                            className={`st0 ${getRegionStyle("포항시_북구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("포항시_북구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <path
                            id="포항시_남구"
                            className={`st0 ${getRegionStyle("포항시_남구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("포항시_남구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
		...
		l-4.1,4.3l-3.6-1.8l-0.5,15.9l-1.9,3.4l-2.7-2.2l-2.1,2.1l-9.5-7.9L90,558l-2.8-1.5l-2.8-1.8l3.2-2.6l-5-0.8l-6-8L76.2,542.4z"/>
                    <g
                        id="무안군그룹" // 그룹 ID
                        className={`${getGroupAnimation(14)} cursor-pointer`} // 무안 cityId는 14
                        onMouseEnter={() => setHoveredSvgId("무안군그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(14); if (city) onCityClick(city); }}
                    >
                        <path
                            id="무안군"
                            className={`st0 ${getRegionStyle("무안군")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("무안군")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
		...
		l-7.8,3.3l0,2.8l-8,0.3l-1.4,0.2l-0.2,0l-3.5,0.5l-0.3,0l-1.1,0.1l-0.2-3l-5.4,0.4l-2.4,0l0.5-8.8L97.4,421.4z"/>
                    <g
                        id="전주시그룹" // 그룹 ID
                        className={`${getGroupAnimation(13)} cursor-pointer`} // 전주 cityId는 13
                        onMouseEnter={() => setHoveredSvgId("전주시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(13); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="전주시_덕진구"
                            className={`st0 ${getRegionStyle("전주시_덕진구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("전주시_덕진구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="전주시_완산구"
                            className={`st0 ${getRegionStyle("전주시_완산구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("전주시_완산구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
		...
		106.4,308.1 112.1,293.4 114.6,294.6 	"/>
                    <g
                        id="홍성군그룹" // 그룹 ID
                        className={`${getGroupAnimation(12)} cursor-pointer`} // 홍성 cityId는 12
                        onMouseEnter={() => setHoveredSvgId("홍성군그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(12); if (city) onCityClick(city); }}
                    >
                        <path
                            id="홍성군"
                            className={`st0 ${getRegionStyle("홍성군")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("홍성군")} // 개별 호버 유지
                            onMouseLeave={() => setHoveredSvgId(null)}   // 개별 호버 유지
                        />
                    </g>
		...
		157,365.5 155.1,359.9 161.6,349.2 154.8,339.3 147.4,341 144.6,337.3 145.4,324.7 	"/>
                    <g
                        id="천안시그룹" // 그룹 ID
                        className={`${getGroupAnimation(22)} cursor-pointer`} // 천안 cityId는 22
                        onMouseEnter={() => setHoveredSvgId("천안시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(22); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="천안시_서북구"
                            className={`st0 ${getRegionStyle("천안시_서북구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("천안시_서북구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="천안시_동남구"
                            className={`st0 ${getRegionStyle("천안시_동남구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("천안시_동남구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
                </g>
		...
		240.1,258.7 244.2,243.8 249.1,238.1 245.7,234.1 	"/>
                    <g
                        id="청주시그룹" // 그룹 ID
                        className={`${getGroupAnimation(11)} cursor-pointer`} // 청주 cityId는 11
                        onMouseEnter={() => setHoveredSvgId("청주시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(11); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="청주시_청원구"
                            className={`st0 ${getRegionStyle("청주시_청원구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("청주시_청원구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="청주시_흥덕구"
                            className={`st0 ${getRegionStyle("청주시_흥덕구")}`}
                            points="198.3,299.1 210,301.4 218.9,311.4 205.7,323.6 200.8,324.5 196,320.8 192.9,312.4 197,305.8 194.2,304.4"
                            onMouseEnter={() => setHoveredSvgId("청주시_흥덕구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="청주시_서원구"
                            className={`st0 ${getRegionStyle("청주시_서원구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("청주시_서원구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="청주시_상당구"
                            className={`st0 ${getRegionStyle("청주시_상당구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("청주시_상당구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
                </g>
		...
		l9.1-2.2l1.3-7.9l0.1,0l1.4-0.6l0-0.1l5.5,2.9l0,7l4.6,6.1L426.2,176.6z"/>
                    <g
                        id="강릉시그룹" // 그룹 ID
                        className={`${getGroupAnimation(20)} cursor-pointer`} // 강릉 cityId는 20
                        onMouseEnter={() => setHoveredSvgId("강릉시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(20); if (city) onCityClick(city); }}
                    >
                        <path
                            id="강릉시"
                            className={`st0 ${getRegionStyle("강릉시")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("강릉시")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
		...
		251.9,222.6 252,222.4 252.2,217.9 253.6,198.6 	"/>
                    <g
                        id="춘천시그룹" // 그룹 ID
                        className={`${getGroupAnimation(10)} cursor-pointer`} // 춘천 cityId는 10
                        onMouseEnter={() => setHoveredSvgId("춘천시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(10); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="춘천시"
                            className={`st0 ${getRegionStyle("춘천시")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("춘천시")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
                    {/* --- ▲▲▲ 춘천시 SVG 코드 종료 ▲▲▲ --- */}
		...
		l-4.8,5.8l-4.5,15.7l-4.3,2.6l0.2,5.3l4.3,0.6l-1.1,6l-5.1,4l-6.4-0.7l-4.7-1.5l-3.2,5.2l-12.4,0.5L118.3,134.9z"/>
                    <g
                        id="용인시그룹" // 그룹 ID
                        className={`${getGroupAnimation(19)} cursor-pointer`} // 용인 cityId는 19
                        onMouseEnter={() => setHoveredSvgId("용인시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(19); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="용인시_처인구"
                            className={`st0 ${getRegionStyle("용인시_처인구")}`}
		...
		197.1,174.8 194.5,175.6 184.1,163.9 180.2,165.1 177.2,155.2 172.8,155.5 169.1,147.1 177.3,137.8 	"/>
                    ...
                    <g
                        id="고양시그룹" // 그룹 ID
                        className={`${getGroupAnimation(21)} cursor-pointer`} // 고양 cityId는 21
                        onMouseEnter={() => setHoveredSvgId("고양시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(21); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="고양시_일산서구"
                            className={`st0 ${getRegionStyle("고양시_일산서구")}`}
                            points="118.5,146.1 130.9,145.6 130.7,150.8 125.7,154.4 118.7,150.3"
                            onMouseEnter={() => setHoveredSvgId("고양시_일산서구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="고양시_일산동구"
                            className={`st0 ${getRegionStyle("고양시_일산동구")}`}
                            points="125.7,154.4 130.7,150.8 130.9,145.6 134.1,140.5 138.8,142 140.9,148.2 133,157.9 129.1,157.2"
                            onMouseEnter={() => setHoveredSvgId("고양시_일산동구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="고양시_덕양구"
                            className={`st0 ${getRegionStyle("고양시_덕양구")}`}
                            ...
                            onMouseEnter={() => setHoveredSvgId("고양시_덕양구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
		...
		159.4,136.6 	"/>
                    <g
                        id="성남시그룹" // 그룹 ID
                        className={`${getGroupAnimation(18)} cursor-pointer`} // 성남 cityId는 18
                        onMouseEnter={() => setHoveredSvgId("성남시그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(18); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="성남시_분당구"
                            className={`st0 ${getRegionStyle("성남시_분당구")}`}
                            points="164.2,192.8 173.7,191 180.5,192.6 176,202.8 172.5,203.7 162.3,197.7"
                            onMouseEnter={() => setHoveredSvgId("성남시_분당구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="성남시_중원구"
                            className={`st0 ${getRegionStyle("성남시_중원구")}`}
                            points="181.2,181.7 183,189 180.5,192.6 173.7,191 180.8,182.2"
                            onMouseEnter={() => setHoveredSvgId("성남시_중원구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                        <polygon
                            id="성남시_수정구"
                            className={`st0 ${getRegionStyle("성남시_수정구")}`}
                            points="174.3,183 176.8,181.9 180.8,182.2 173.7,191 164.2,192.8 164.1,190.9 164.7,188.6 170.7,184.6"
                            onMouseEnter={() => setHoveredSvgId("성남시_수정구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                        />
                    </g>
                    {/* --- ▲▲▲ 성남시 SVG 코드 종료 ▲▲▲ --- */}
                    <g
                        id="수원시" // 그룹 ID
                        className={`${getGroupAnimation(2)} cursor-pointer`} // cityId 2번에 대한 애니메이션
                        onMouseEnter={() => setHoveredSvgId("수원시_그룹")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => { const city = getCityData(2); if (city) onCityClick(city); }}
                    >
                        <polygon
                            id="수원시_영통구"
                            // className은 원래대로 동적 스타일 적용
                            className={`st0 ${getRegionStyle("수원시_영통구")}`}
                            points="169.9,209.3 167,218.4 163.4,218.6 163.3,215.8 163,210 163.7,205.7"
                            // onMouseEnter/Leave를 개별 ID로 다시 설정
                            onMouseEnter={() => setHoveredSvgId("수원시_영통구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                            // onClick은 handleRegionClick 호출 유지
                            onClick={() => handleRegionClick("수원시_영통구")}
                        />
                        <polygon
                            id="수원시_팔달구"
                            className={`st0 ${getRegionStyle("수원시_팔달구")}`}
                            points="163,210 163.3,215.8 156,211.1"
                            onMouseEnter={() => setHoveredSvgId("수원시_팔달구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                            onClick={() => handleRegionClick("수원시_팔달구")}
                        />
                        <polygon
                            id="수원시_권선구"
                            className={`st0 ${getRegionStyle("수원시_권선구")}`}
                            points="163.4,218.6 155,220 149.6,212.2 150.3,208.5 153.8,208.6 156,211.1 163.3,215.8"
                            onMouseEnter={() => setHoveredSvgId("수원시_권선구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                            onClick={() => handleRegionClick("수원시_권선구")}
                        />
                        <polygon
                            id="수원시_장안구"
                            className={`st0 ${getRegionStyle("수원시_장안구")}`}
                            points="161,201 163.7,205.5 163.7,205.5 163.7,205.7 163,210 156,211.1 153.8,208.6 153.8,208.6 155.2,204"
                            onMouseEnter={() => setHoveredSvgId("수원시_장안구")}
                            onMouseLeave={() => setHoveredSvgId(null)}
                            onClick={() => handleRegionClick("수원시_장안구")}
                        />
                    </g>
		...
		133.6,209.3 135.8,209.1 136.8,208.2 139,207.8 139,199 139.1,198.3 133.2,200.2 131.8,203.1 128.8,202.5 	"/>
                ...
                <g
                    id="세종시그룹" // 그룹 ID (SVG 원본 ID 대신 사용)
                    className={`${getGroupAnimation(9)} cursor-pointer`} // 세종 cityId는 9
                    onMouseEnter={() => setHoveredSvgId("세종시그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(9); if (city) onCityClick(city); }}
                >
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="세종특별자치시"
                        className={`st0 ${getRegionStyle("세종특별자치시")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("세종특별자치시")} // 개별 호버는 유지
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                {/* --- ▲▲▲ 세종특별자치시 SVG 코드 종료 ▲▲▲ --- */}
                <g
                    id="울산광역시" // 그룹 ID
                    className={`${getGroupAnimation(8)} cursor-pointer`} // 울산 cityId는 8
                    onMouseEnter={() => setHoveredSvgId("울산광역시_그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(8); if (city) onCityClick(city); }}
                >
                    <path
                        id="울주군"
                        className={`st0 ${getRegionStyle("울주군")}`}
                ...
                <g
                    id="대전광역시" // 그룹 ID
                    className={`${getGroupAnimation(7)} cursor-pointer`} // 대전 cityId는 7
                    onMouseEnter={() => setHoveredSvgId("대전광역시_그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(7); if (city) onCityClick(city); }}
                >
                    <polygon
                        id="대덕구"
                        className={`st0 ${getRegionStyle("대덕구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("대덕구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="유성구"
                        className={`st0 ${getRegionStyle("유성구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("유성구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="서구_대전"
                        className={`st0 ${getRegionStyle("서구_대전")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("서구_대전")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="중구_대전"
                        className={`st0 ${getRegionStyle("중구_대전")}`}
                        points="210.4,359.1 216,366.7 212.1,366.8 214,375.1 211.4,379.9 207,372.7 205,369.9 204.8,367.2 209,357.8"
                        onMouseEnter={() => setHoveredSvgId("중구_대전")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="동구_대전"
                        className={`st0 ${getRegionStyle("동구_대전")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("동구_대전")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                {/* --- ▲▲▲ 대전광역시 SVG 코드 종료 ▲▲▲ --- */}
                <g
                    id="광주광역시" // 그룹 ID
                    className={`${getGroupAnimation(6)} cursor-pointer`} // 광주 cityId는 6
                    onMouseEnter={() => setHoveredSvgId("광주광역시_그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(6); if (city) onCityClick(city); }}
                >
                    <polygon
                        id="광산구"
                        className={`st0 ${getRegionStyle("광산구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("광산구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="북구_광주"
                        className={`st0 ${getRegionStyle("북구_광주")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("북구_광주")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="남구_광주"
                        className={`st0 ${getRegionStyle("남구_광주")}`}
                        points="131.6,553.9 142.6,550.3 142.3,545.7 145.4,545.3 146.6,554.3 143.1,556.4 133.5,560.3 125.6,559.4 127.5,557.5"
                        onMouseEnter={() => setHoveredSvgId("남구_광주")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="서구_광주"
                        className={`st0 ${getRegionStyle("서구_광주")}`}
                        points="136.3,540.6 145.1,544.9 145.4,545.3 142.3,545.7 142.6,550.3 131.6,553.9"
                        onMouseEnter={() => setHoveredSvgId("서구_광주")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="동구_광주"
                        className={`st0 ${getRegionStyle("동구_광주")}`}
                        points="145.1,544.9 149.1,543.3 157.5,549.7 150.7,557.2 146.6,554.3 145.4,545.3"
                        onMouseEnter={() => setHoveredSvgId("동구_광주")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                <g
                    id="대구광역시" // 그룹 ID
                    className={`${getGroupAnimation(5)} cursor-pointer`} // 대구 cityId는 5
                    onMouseEnter={() => setHoveredSvgId("대구광역시_그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(5); if (city) onCityClick(city); }}
                >
                    <path
                        id="달성군"
                        className={`st0 ${getRegionStyle("달성군")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("달성군")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="달서구"
                        className={`st0 ${getRegionStyle("달서구")}`}
                        points="350.9,432.2 357.9,433.2 358,433.6 355.6,437.6 359.7,441.8 355.6,447.2 346.4,440.7 345,437.9 346.1,432.4"
                        onMouseEnter={() => setHoveredSvgId("달서구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="수성구"
                        className={`st0 ${getRegionStyle("수성구")}`}
                        points="360.2,441.5 362.3,434.3 363.1,432.3 377.2,433.7 376.3,441.7 371.8,444.4 363.9,442"
                        onMouseEnter={() => setHoveredSvgId("수성구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="북구" // 대구 북구 ID 확인 필요 (북구_대구? 그냥 북구?) - 우선 "북구" 사용
                        className={`st0 ${getRegionStyle("북구")} `} // Map Key는 "북구" 그대로 사용
                        ...
                        onMouseEnter={() => setHoveredSvgId("북구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="남구" // 대구 남구 ID 확인 필요 (남구_대구? 그냥 남구?) - 우선 "남구" 사용
                        className={`st0 ${getRegionStyle("남구")}`} // Map Key는 "남구" 그대로 사용
                        points="358,433.6 362.3,434.3 360.2,441.5 359.7,441.8 355.6,437.6"
                        onMouseEnter={() => setHoveredSvgId("남구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="서구_대구"
                        className={`st0 ${getRegionStyle("서구_대구")}`}
                        points="351.7,429.3 358.6,430.6 357.9,433.2 350.9,432.2"
                        onMouseEnter={() => setHoveredSvgId("서구_대구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="동구_대구"
                        className={`st0 ${getRegionStyle("동구_대구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("동구_대구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="중구_대구"
                        className={`st0 ${getRegionStyle("중구_대구")}`}
                        points="358.6,430.6 362.5,431.2 363.1,432.3 362.3,434.3 358,433.6 357.9,433.2"
                        onMouseEnter={() => setHoveredSvgId("중구_대구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                <g
                    id="인천광역시" // 그룹 ID
                    // 그룹 태그에 조건부 애니메이션 클래스와 커서 추가
                    className={`${getGroupAnimation(3)} cursor-pointer`} // 인천 cityId는 3
                    // 그룹 호버 시 상태 업데이트 (툴팁용)
                    onMouseEnter={() => setHoveredSvgId("인천광역시_그룹")} // 그룹 식별용 고유 ID
                    onMouseLeave={() => setHoveredSvgId(null)}
                    // 그룹 클릭 시 cityId 3번 데이터 전달
                    onClick={() => {
                        const city = getCityData(3);
                        if (city) onCityClick(city);
                    }}
                >
                    {/* 그룹 안의 path/polygon들은 className(스타일)과 개별 호버 이벤트만 가짐 */}
                    <path
                        id="강화군"
                        className={`st0 ${getRegionStyle("강화군")}`} // 애니메이션 제거
                        ...
                        onMouseEnter={() => setHoveredSvgId("강화군")} // 개별 호버 유지
                        onMouseLeave={() => setHoveredSvgId(null)}   // 개별 호버 유지
                    />
                    <path
                        id="서구"
                        className={`st0 ${getRegionStyle("서구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("서구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="계양구"
                        className={`st0 ${getRegionStyle("계양구")}`}
                        points="124.8,163.1 133.3,164.8 129.8,169 129,173.7 120.2,173.7 123.1,169.2 119.4,164.3"
                        onMouseEnter={() => setHoveredSvgId("계양구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="부평구"
                        className={`st0 ${getRegionStyle("부평구")}`}
                        points="129,173.7 126.8,179.5 129.9,181.9 120,182 120.2,173.7"
                        onMouseEnter={() => setHoveredSvgId("부평구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="남동구"
                        className={`st0 ${getRegionStyle("남동구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("남동구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="연수구"
                        className={`st0 ${getRegionStyle("연수구")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("연수구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <path
                        id="중구_인천"
                        className={`st0 ${getRegionStyle("중구_인천")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("중구_인천")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <path
                        id="옹진군"
                        className={`st0 ${getRegionStyle("옹진군")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("옹진군")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="동구"
                        className={`st0 ${getRegionStyle("동구")}`}
                        points="113.6,178 118.3,181.2 114.3,182.3 110.5,179.9 110.3,179.3 111.1,178.9 111.2,178.9 112.3,178.5"
                        onMouseEnter={() => setHoveredSvgId("동구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="미추홀구"
                        className={`st0 ${getRegionStyle("미추홀구")}`}
                        points="114.3,182.3 118.3,181.2 119.7,181.9 121.7,186.9 113.3,188.2 112.8,187.2 112.7,186.5"
                        onMouseEnter={() => setHoveredSvgId("미추홀구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                <g
                    id="부산광역시" // 그룹 ID
                    className={`${getGroupAnimation(4)} cursor-pointer`} // 부산 cityId는 4
                    onMouseEnter={() => setHoveredSvgId("부산광역시_그룹")}
                    onMouseLeave={() => setHoveredSvgId(null)}
                    onClick={() => { const city = getCityData(4); if (city) onCityClick(city); }}
                >
                    <path
                        id="기장군"
                        className={`st0 ${getRegionStyle("기장군")}`}
                        d="M452.4,515.1l-5.5,2.4l-6.4,21l-6.3-3.7l-2.1-6.9l-4.8-8.3l3.2-7.4l-2.1-2.7l7.7,2.6l2.9-5.7 l8.2,0L452.4,515.1z"
                        onMouseEnter={() => setHoveredSvgId("기장군")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="사상구"
                        className={`st0 ${getRegionStyle("사상구")}`}
                        points="412.1,535.9 416.4,538 415.4,545.8 413.9,548.1 408.7,549 408.7,546.1"
                        onMouseEnter={() => setHoveredSvgId("사상구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="수영구"
                        className={`st0 ${getRegionStyle("수영구")}`}
                        points="428.6,538 430.1,541.2 430,541.3 430.7,542.1 429,545.2 426.1,541.9 428.5,538.4"
                        onMouseEnter={() => setHoveredSvgId("수영구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="연제구"
                        className={`st0 ${getRegionStyle("연제구")}`}
                        points="419.9,536.8 428.5,538.4 426.1,541.9 424.4,542.5"
                        onMouseEnter={() => setHoveredSvgId("연제구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <path
                        id="강서구_부산"
                        className={`st0 ${getRegionStyle("강서구_부산")}`}
                        ...
                        onMouseEnter={() => setHoveredSvgId("강서구_부산")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="금정구"
                        className={`st0 ${getRegionStyle("금정구")}`}
                        points="432.1,527.8 427.8,534.3 421.6,531.9 419.3,531.2 419.6,524.4 427.4,519.5"
                        onMouseEnter={() => setHoveredSvgId("금정구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <path
                        id="사하구"
                        className={`st0 ${getRegionStyle("사하구")}`}
                        d="M407.7,554.9l-2.4-1.9l0-0.4l0.3-1.6l0-0.1l2.9-4.9l0,3l5.2-0.9l1.1,11.6l-4.4-1.2l-0.1,3 l-2-3.3L407.7,554.9z"
                        onMouseEnter={() => setHoveredSvgId("사하구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <path
                        id="해운대구"
                        className={`st0 ${getRegionStyle("해운대구")}`}
                        d="M427.8,534.3l4.3-6.5l2.1,6.9l6.3,3.7l-1.8,3.3l-6.6,1.1l-0.9-1l-1.1-0.6l-1.5-3.2 L427.8,534.3z"
                        onMouseEnter={() => setHoveredSvgId("해운대구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="북구_부산"
                        className={`st0 ${getRegionStyle("북구_부산")}`}
                        points="412.1,535.9 413.2,530.5 415.3,524.5 419.6,524.4 419.3,531.2 421.6,531.9 419.4,536.1 416.4,538"
                        onMouseEnter={() => setHoveredSvgId("북구_부산")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <path
                        id="남구_부산"
                        className={`st0 ${getRegionStyle("남구_부산")}`}
                        d="M426.3,552.5l-0.4-2.4l-3.4-0.3l-0.1-2.6 l-0.3-2.1l2.3-2.7l1.6-0.5l3.4,4l0.4,5.6L426.3,552.5z"
                        onMouseEnter={() => setHoveredSvgId("남구_부산")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="동래구"
                        className={`st0 ${getRegionStyle("동래구")}`}
                        points="421.6,531.9 427.8,534.3 428.6,538 428.5,538.4 419.9,536.8 419.4,536.1"
                        onMouseEnter={() => setHoveredSvgId("동래구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <polygon
                        id="부산진구"
                        className={`st0 ${getRegionStyle("부산진구")}`}
                        points="416.4,538 419.4,536.1 419.9,536.8 424.4,542.5 422.1,545.1 417.2,546.1 415.4,545.8"
                        onMouseEnter={() => setHoveredSvgId("부산진구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    <path
                        id="영도구"
                        className={`st0 ${getRegionStyle("영도구")}`}
                        d="M424,553.6l1.9,5.5l-7.7-5.1l-0.2-1.2l0.2-0.3l0.9-0.2l0.1,0l0.5-0.2L424,553.6L424,553.6z"
                        onMouseEnter={() => setHoveredSvgId("영도구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="동구_부산"
                        className={`st0 ${getRegionStyle("동구_부산")}`}
                        points="417.2,546.1 422.1,545.1 422.2,547.2 422.2,547.3 420.3,549.6 417.4,549.2"
                        onMouseEnter={() => setHoveredSvgId("동구_부산")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <path
                        id="서구_부산"
                        className={`st0 ${getRegionStyle("서구_부산")}`}
                        d="M417,555.5l-0.2,2.3l-1.7,1.7l-1.3-11.3 l1.6-2.2l1.7,0.2l0.3,3.2l0,3.9l0.5,0.7L417,555.5z"
                        onMouseEnter={() => setHoveredSvgId("서구_부산")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                    {/* ID 짧게 수정됨 */}
                    <polygon
                        id="중구_부산"
                        className={`st0 ${getRegionStyle("중구_부산")}`}
                        points="417.4,552.5 418.6,552.2 420.3,549.6 417.4,549.2"
                        onMouseEnter={() => setHoveredSvgId("중구_부산")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    />
                </g>
                {/* --- ▼▼▼ 서울특별시 SVG 코드 시작 (그룹 애니메이션 및 클릭 적용) ▼▼▼ --- */}
                <g
                    id="서울특별시" // 그룹 ID
                    // 그룹 태그에 조건부 애니메이션 클래스와 커서 추가
                    className={`${getGroupAnimation(1)} cursor-pointer`} // 서울 cityId는 1
                    // 그룹 호버 시 상태 업데이트 (툴팁용)
                    onMouseEnter={() => setHoveredSvgId("서울특별시_그룹")} // 그룹 식별용 고유 ID
                    onMouseLeave={() => setHoveredSvgId(null)}
                    // 그룹 클릭 시 cityId 1번 데이터 전달
                    onClick={() => {
                        const city = getCityData(1);
                        if (city) onCityClick(city);
                    }}
                >
                    <polygon
                        id="강동구"
                        className={`st0 ${getRegionStyle("강동구")}`}
                        points="179.9,165.4 181.8,170.7 177,175.1 172.5,171 173.3,168.9"
                        onMouseEnter={() => setHoveredSvgId("강동구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("강동구")} // 공통 핸들러 사용
                    />
                    <polygon
                        id="송파구" // ID 확인 ("송파구"는 svgIdToCityIdMap에 있음, cityId: 1)
                        className={`st0 ${getRegionStyle("송파구")}`} // className 수정 및 동적 스타일 추가
                        points="172.5,171 177,175.1 176.8,181.9 174.3,183 172.5,178.7 168.3,177.4 167.3,173.9 171.3,172.4" // points는 그대로
                        onMouseEnter={() => setHoveredSvgId("송파구")} // 이벤트 핸들러 추가
                        onMouseLeave={() => setHoveredSvgId(null)} // 이벤트 핸들러 추가
                        onClick={() => handleRegionClick("송파구")} // 이벤트 핸들러 추가
                    />
                    {/* 강남구 */}
                    <polygon
                        id="강남구"
                        className={`st0 ${getRegionStyle("강남구")} `}
                        ...
                        onMouseEnter={() => setHoveredSvgId("강남구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("강남구")}
                    />

                    {/* 서초구 */}
                    <polygon
                        id="서초구"
                        className={`st0 ${getRegionStyle("서초구")} `}
                        points="160,173.7 165.2,182.9 169.5,181.6 170.7,184.6 164.7,188.6 163.2,183.3 157.4,184.2 156.6,181.4 156.5,176.7"
                        onMouseEnter={() => setHoveredSvgId("서초구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("서초구")}
                    />

                    {/* 관악구 */}
                    <polygon
                        id="관악구"
                        className={`st0 ${getRegionStyle("관악구")} `}
                        points="146.8,180 153.9,178.7 156.6,181.4 157.4,184.2 154.3,186.9 152.4,187.3 149.9,185.4 146.3,180.9 146.8,180"
                        onMouseEnter={() => setHoveredSvgId("관악구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("관악구")}
                    />
                    {/* 동작구 */}
                    <polygon
                        id="동작구"
                        className={`st0 ${getRegionStyle("동작구")} `}
                        points="152.7,174.9 156.5,176.7 156.6,181.4 153.9,178.7 146.8,180"
                        onMouseEnter={() => setHoveredSvgId("동작구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("동작구")}
                    />

                    {/* 영등포구 */}
                    <polygon
                        id="영등포구"
                        className={`st0 ${getRegionStyle("영등포구")} `}
                        points="144.1,168.8 152,172.4 152.7,174.9 146.8,180 146.8,180 143.9,174.8 144.8,171.4 144.1,170.1"
                        onMouseEnter={() => setHoveredSvgId("영등포구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("영등포구")}
                    />

                    {/* 금천구 */}
                    <polygon
                        id="금천구"
                        className={`st0 ${getRegionStyle("금천구")} `}
                        points="143.2,179.9 146.3,180.9 149.9,185.4 146.2,187.2"
                        onMouseEnter={() => setHoveredSvgId("금천구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("금천구")}
                    />

                    {/* 구로구 */}
                    <polygon
                        id="구로구"
                        className={`st0 ${getRegionStyle("구로구")} `}
                        points="143.9,174.8 146.8,180 146.3,180.9 143.2,179.9 138.1,181.1 135.8,181.5 137.3,176.2"
                        onMouseEnter={() => setHoveredSvgId("구로구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("구로구")}
                    />

                    {/* 강서구 */}
                    <polygon
                        id="강서구"
                        className={`st0 ${getRegionStyle("강서구")} `}
                        points="134.4,161.1 140.7,166.4 144.1,168.8 144.1,170.1 139,173.4 136.8,171.2 129.8,169 133.3,164.8"
                        onMouseEnter={() => setHoveredSvgId("강서구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    ...
                    <polygon
                        id="양천구"
                        className={`st0 ${getRegionStyle("양천구")} `}
                        points="136.8,171.2 139,173.4 144.1,170.1 144.8,171.4 144.8,171.4 143.9,174.8 137.3,176.2"
                        onMouseEnter={() => setHoveredSvgId("양천구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("양천구")}
                    />
                    {/* 마포구 */}
                    <polygon
                        id="마포구"
                        className={`st0 ${getRegionStyle("마포구")} `}
                        points="144.3,163.4 146.8,165.7 154.2,168.4 154.2,169.6 152,172.4 144.1,168.8 140.7,166.4"
                        onMouseEnter={() => setHoveredSvgId("마포구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("마포구")}
                    />

                    {/* 서대문구 */}
                    <polygon
                        id="서대문구"
                        className={`st0 ${getRegionStyle("서대문구")} `}
                        points="152.8,160.4 154.8,167.4 154.2,168.4 146.8,165.7"
                        onMouseEnter={() => setHoveredSvgId("서대문구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("서대문구")}
                    />

                    {/* 은평구 */}
                    <polygon
                        id="은평구"
                        className={`st0 ${getRegionStyle("은평구")} `}
                        points="144.3,163.4 146.8,162.7 147.2,154.3 152.5,152.8 153.8,157.4 152.8,160.4 146.8,165.7"
                        onMouseEnter={() => setHoveredSvgId("은평구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("은평구")}
                    />

                    {/* 노원구 */}
                    <polygon
                        id="노원구"
                        className={`st0 ${getRegionStyle("노원구")} `}
                        points="172.8,155.5 172.1,158.9 167.7,159.7 165.2,158.3 164.3,157.3 165.5,148.7 169.1,147.1"
                        onMouseEnter={() => setHoveredSvgId("노원구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("노원구")}
                    />

                    {/* 도봉구 */}
                    <polygon
                        id="도봉구"
                        className={`st0 ${getRegionStyle("도봉구")} `}
                        points="164.3,157.3 160.1,148.9 161,146.2 165.5,148.7"
                        onMouseEnter={() => setHoveredSvgId("도봉구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("도봉구")}
                    />

                    {/* 강북구 */}
                    <polygon
                        id="강북구"
                        className={`st0 ${getRegionStyle("강북구")} `}
                        points="158.1,149.6 160.1,148.9 164.3,157.3 165.2,158.3 162.7,160.7 157.3,156.5 157.1,156.4 157,156.4"
                        onMouseEnter={() => setHoveredSvgId("강북구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("강북구")}
                    />구
                    {/* 성북구 */}
                    <polygon
                        id="성북구"
                        className={`st0 ${getRegionStyle("성북구")} `}
                        ...
                        onMouseEnter={() => setHoveredSvgId("성북구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("성북구")}
                    />

                    {/* 중랑구 */}
                    <polygon
                        id="중랑구"
                        className={`st0 ${getRegionStyle("중랑구")} `}
                        points="172.1,158.9 171.5,166.2 168.7,166.5 168.7,166.5 168.6,166.3 168.6,166.3 167.8,160.9 167.7,159.7"
                        onMouseEnter={() => setHoveredSvgId("중랑구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("중랑구")}
                    />

                    {/* 동대문구 */}
                    <polygon
                        id="동대문구"
                        className={`st0 ${getRegionStyle("동대문구")} `}
                        points="167.8,160.9 168.6,166.3 168.7,166.5 168.4,167.3 168.4,167.3 168,168.4 161.9,166.5 161.9,166.5 161.8,165.5"
                        onMouseEnter={() => setHoveredSvgId("동대문구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("동대문구")}
                    />
                    <polygon
                        id="광진구"
                        className={`st0 ${getRegionStyle("광진구")} `}
                        ...
                        onMouseEnter={() => setHoveredSvgId("광진구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("광진구")}
                    />

                    {/* 용산구 */}
                    <polygon
                        id="용산구"
                        className={`st0 ${getRegionStyle("용산구")} `}
                        points="154.2,169.6 160,170.8 161.1,172.4 160,173.7 156.5,176.7 152.7,174.9 152,172.4"
                        onMouseEnter={() => setHoveredSvgId("용산구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("용산구")}
                    />

                    {/* 중구 */}
                    <polygon
                        id="중구"
                        className={`st0 ${getRegionStyle("중구")} `}
                        points="154.8,167.4 161.9,166.5 161.9,166.5 160,170.8 154.2,169.6 154.2,168.4"
                        onMouseEnter={() => setHoveredSvgId("중구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                    ...
                    <polygon
                        id="종로구"
                        className={`st0 ${getRegionStyle("종로구")} `}
                        points="155.9,157.1 156.1,162.4 161.8,165.5 161.8,165.5 161.9,166.5 154.8,167.4 152.8,160.4 153.8,157.4"
                        onMouseEnter={() => setHoveredSvgId("종로구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("종로구")}
                    />

                    {/* 성동구 */}
                    <polygon
                        id="성동구"
                        className={`st0 ${getRegionStyle("성동구")} `}
                        points="161.9,166.5 168,168.4 165.9,173.3 161.1,172.4 160,170.8"
                        onMouseEnter={() => setHoveredSvgId("성동구")}
                        onMouseLeave={() => setHoveredSvgId(null)}
                        onClick={() => handleRegionClick("성동구")}
                    />
                </g>
            ...
            {/* Tooltip */}
            {hoveredSvgId && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/70 backdrop-blur-sm px-4 py-2 rounded-lg text-sm font-medium pointer-events-none shadow-lg">
                    { cities.find(c => c.cityId === getCityIdFromSvgId(hoveredSvgId))?.cityName }
                </div>
            )}
        </div>
    );
